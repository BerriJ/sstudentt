...
class SST:
    ...
    def __init__(self, mu, sigma, nu, tau):
        ...
        self.c = 2 * self.nu * ((1 + self.nu ** 2) * beta(0.5, self.tau / 2) * self.tau ** 0.5) ** -1
        self.m = ((2 * self.tau ** 0.5) * (self.nu - self.nu ** -1)) / ((self.tau - 1) * beta(0.5, 0.5 * self.tau))
        self.s2 = ((self.tau / (self.tau - 2)) * (self.nu ** 2 + self.nu ** -2 - 1) - self.m ** 2)
    ...
    def q(self, p):
        ...
        quantile = np.where(p <= (1 + nu ** 2) ** -1,
                            mu_0 + (sigma_0 / nu) * stats.t.ppf((p * (1 + nu ** 2)) * 0.5, tau),
                            mu_0 + sigma_0 * nu * stats.t.ppf((p * (1 + nu ** 2) - 1 + nu ** 2) / (2 * nu ** 2), tau)
                            )
        return quantile
